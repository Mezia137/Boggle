...
class digit_tree():
	def __init__(self, lettre, parent=None):
		self.lettre = lettre
		self.parent = parent
		self.child = [np.full(shape=25, fill_value=None)]
		self.word = False
	...
	with open(input_file, 'r') as words:
		for word in words:
			...
			for i, lettre in enumerate(word[:-1]):
				...
				#if branch.child[lettre_id] == None:
				if not isinstance(branch.child[lettre_id], np.ndarray):
					branch.child[lettre_id] = digit_tree(lettre_id, parent=branch)
